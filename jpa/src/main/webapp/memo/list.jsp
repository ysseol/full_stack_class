        ...
        <table class="table table-hover table-bordered align-middle text-center">
            <thead class="table-light">
                <tr>
                    ...
                    <th>날짜</th>
                    <th>
                        <button type="button" id="btnAllDel" class="btn btn-sm btn-danger">선택삭제</button>
                    </th>
                </tr>
            </thead>
			<tbody>
			<c:forEach var="row" items="${list}">
		    	<tr>
		        	...
		        	<td>${row.writer}</td>
		        	<td>
		            	<a href="<%=request.getContextPath()%>/memo_servlet/view.do?idx=${row.idx}" class="text-decoration-none">
		                	${row.memo}
		            	</a>
		        	</td>
		            <td>${row.post_date}</td>
		            <td>
		            	<button type="button" class="btn btn-sm btn-outline-danger" onclick="memo_del('${row.idx}')">삭제</button>
		            </td>
		    	</tr>
			</c:forEach>
			</tbody>
